...
void USART2_Config(u32 bound)
{
  ...
  USART_Init(USART2, &USART_InitStructure); //?????	2
	...
	NVIC_Init(&NVIC_InitStructure);	//USART2 ???????
	
	
	USART_ITConfig(USART2, USART_IT_RXNE ,ENABLE);   //??????
	USART_ClearITPendingBit(USART2, USART_IT_RXNE);   //???????
	USART_Cmd(USART2, ENABLE);                                     //1  2  3  ??
}
void USART3_Config(u32 bound)    // USART3 ??? ??IO?
{
	//GPIO????
  GPIO_InitTypeDef GPIO_InitStructure;
	USART_InitTypeDef USART_InitStructure;
	...
	RCC_APB1PeriphClockCmd(RCC_APB1Periph_USART3, ENABLE);	//??USART1,GPIOA??
  ...
  //USART3_RX	  
	  ...
	  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN_FLOATING;//????
  GPIO_Init(GPIOB, &GPIO_InitStructure);//???GP

  //Usart1 NVIC ??
    NVIC_InitStructure.NVIC_IRQChannel = USART3_IRQn;
	NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority=2 ;//?????3s
  ...
  {     
    dat = USART_ReceiveData(USART2);  
	 ...
	 {  			
      ...
      USART_Send_Char(USART2,0x77); 
	  USART_Send_Char(USART2,0x97);                             			  
	 }   
		USART_Send_Char(USART2,dat-1); 
  }
} 
