...
void UartRxMonitor(u8 ms) 					
{
	...
	if(modbus.Modbus_RX_Cnt > 0)					//接收计数器大于零时，监控总线空闲时间
	{
		...
		else                              			//接收计数器未改变，即总线空闲时，累计空闲时间
		{
			...
			if(idletmr < 1)                 		//空闲时间小于3ms时，持续累加
			{
				idletmr += ms;
				if(idletmr >= 1)            		//空闲时间达到3ms时，即判定为1帧接收完毕
				{
    ...
    if(func_code == 0x03)
    {
		...
		switch(slave_addr)
		{
			case 0x01:
				...
				break;
			case 0x02:
				DataReady = 2;   // 设置数据就绪标志
				break;
			case 0x03:
				DataReady = 3;   // 设置数据就绪标志
				break;
			case 0x04:
				DataReady = 4;   // 设置数据就绪标志
				break;
		}
    }
}
